...
def get_topncity():
    try:     
        ...
        n: int = int(payload['n'])
        user_list, top_n_cities = [], {}
        if rows and n:
            for row in rows:
                if len(row.city) > 0:
                    user_list.append(row.city)

            top_n = FindTopN(user_list, n)
            for user, freq in top_n:
                top_n_cities[user] = freq

            resp = HttpResponse(data=top_n_cities, status=200, message='Successfully Listed Top {0} Cities'.format(n))
        else:
            resp = HttpResponse(status=500, message='Failed to List Top {0} Cities'.format(n))
