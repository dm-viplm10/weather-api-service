...
def Forecast():
    try:
        ...
        username: str = payload.get('user_name', None)
        user_obj = (
            db.session.query(User)
            .filter(User.username == username)
            .first()
        )
        login = user_service.login_user(payload)
        #print(login.status, city, country, days)
        if login.status == 200 and city and isinstance(days, int):
            ...
            if response.status_code == 200:
                audit = AuditActivity(datetime.now(), data['location']['country'], data['location']['name'], user_obj, 'Successfully Fetched Forecast')
                ...
                try:
                    ...
                    audit = AuditActivity(datetime.now(), '', '', user_obj, data['error']['message'])
                    db.session.add(audit)
                    db.session.commit()
                except Exception as exc:
                    traceback.print_exc()
                    resp = HttpResponse(message='Exception Occured - Invalid API request', status=response.status_code)
                    audit = AuditActivity(datetime.now(), '', '', user_obj, 'Exception Occured - '+str(e))
                    db.session.add(audit)
                    db.session.commit()
